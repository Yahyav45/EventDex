...
def approvebookingrequest(request,id):
    res=Bookingrequest.objects.filter(id=id).update(status='approved')
...
def viewvendorreview(request):
    ...
    r=Vendorreview.objects.filter(VENDOR=vid)
    l=[]
    for i in r:
        l.append({"id":i.id,
                  "date":i.date,
...
def userviewvendor(request):
    search=request.POST['search']
    r = Vendor.objects.filter((Q(vendorname__icontains=search) | Q(category__icontains=search)| Q(place__icontains=search)) & Q(status='approve'))
    l = []
    for i in r:
        l.append({"id": i.id,
                  "vendorname": i.vendorname,
...
def addbooking_post(request):
    lid = request.POST['lid']
    eventdate = request.POST["textfield"]
    service=request.POST["sid"]
...
def userpayment(request):
    ...
    obj.REQUEST_id=bid
    obj.amount=amount
    obj.paymentstatus='Booking Confirmed'
    obj.save()

    Bookingrequest.objects.filter(id=bid).update(status="Booking Confirmed")
